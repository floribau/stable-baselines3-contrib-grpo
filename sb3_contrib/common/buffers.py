...
class BaseGroupBuffer(BaseBuffer, ABC):
    """
...
class DeepSeekProcessGroupBuffer(BaseGroupBuffer):
    """
    Buffer class containing a group of trajectories for a single GRPO update.
...
class DeepSeekOutcomeGroupBuffer(BaseGroupBuffer):
    """
    Buffer class containing a group of trajectories for a single GRPO update.
    ...
    def get_leave_one_out_advantages(self) -> list[th.Tensor]:
        ...
        k = len(self.returns)
        advantages = [r - (total_return_sum - r) / (k - 1) for r in self.returns]
        if self.scale_rewards:
            std_return = th.stack(self.returns).std()
