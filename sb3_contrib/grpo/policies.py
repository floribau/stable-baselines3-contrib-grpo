"""Module providing policy implementations for GRPO."""
...
class ActorPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if isinstance(net_arch, list) and len(net_arch) > 0 and isinstance(net_arch[0], dict):
            warnings.warn(
                (
                    ...
                    "(net_arch=dict(pi=...) instead of net_arch=[dict(pi=...)])"
                ),
            )
            net_arch = net_arch[0]
        ...
        self.pi_features_extractor = self.features_extractor  # ensure naming consistency with SB3
        ...
        self.mlp_extractor: MlpExtractor = None
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        """
        Create the actor network and the optimizer.
    ...
    def extract_features(  # type: ignore[override]
        ...
        :return: The extracted features for the actor.
        """
    ...
    def evaluate_actions(self, obs: PyTorchObs, actions: th.Tensor) -> tuple[th.Tensor, th.Tensor | None]:
        """
        ...
        :return: Tuple containing the log likelihood of taking those actions and the entropy of the action distribution.
        """
        # Preprocess the observation if needed
        pi_features = self.extract_features(obs)
        latent_pi = self.mlp_extractor.forward_actor(pi_features)
    ...
    def get_frozen_deepcopy(self) -> "ActorPolicy":
        """
        Get a frozen deepcopy of the current policy state.

        :return: A frozen deepcopy of the policy.
        """
        frozen_policy = deepcopy(self)
        frozen_policy.set_training_mode(False)
        for param in frozen_policy.parameters():
            param.requires_grad = False

        return frozen_policy

    # NOTE this should probably be removed, as it doesn't solve my problem
    @classmethod
